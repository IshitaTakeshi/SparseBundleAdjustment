...
def optimize_scipy(sba, observation):
    mask = np.logical_not(np.isnan(observation))

    def masked_diff(x1, x2):
        return x1[mask] - x2[mask]

    def error(p):
        d = masked_diff(sba.projection(p), observation)
        return np.dot(d, d)
...
def main():
    ...
    points3d, poses = optimize_scipy(sba, observation)
